...
<template>
    <div class="mt-4 flex flex-col gap-4">
        <div>
            ...
            <DxDataGrid :data-source="ownAdviceStatusSource" :cell-hint-enabled="true" @saved="reload">
                <DxEditing :allow-updating="true" :allow-adding="true" :allow-deleting="true" mode="row" />
                <DxColumn data-field="name" caption="Name" sort-order="asc" />
                <DxColumn data-field="result" caption="Ergebnis">
                    <DxLookup :data-source="adviceStatusResult" display-expr="name" value-expr="id" />
                </DxColumn>
                <DxColumn data-field="group_id" caption="Initiative" :visible="false" :allow-editing="false" :default-value="props.group.id" />
            </DxDataGrid>
        </div>
        <div>
            ...
            <DxDataGrid :data-source="otherAdviceStatusSource" :cell-hint-enabled="true" @saved="reload">
                <DxEditing :allow-updating="true" :allow-adding="false" :allow-deleting="false" mode="cell" />
                <DxColumn data-field="name" caption="Name" :allow-editing="false" sort-order="asc" />
                <DxColumn data-field="group_id" caption="Initiative" :allow-editing="false">
                    <DxLookup :data-source="groupsSource" display-expr="name" value-expr="id" />
                </DxColumn>
                <DxColumn data-field="visible_in_group" caption="Sichtbar" data-type="boolean" @cell-value-changed="onVisibilityChanged" />
            </DxDataGrid>
        </div>
    </div>
</template>

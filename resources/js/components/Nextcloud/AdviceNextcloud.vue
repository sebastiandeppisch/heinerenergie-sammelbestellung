...
watch(() => props.advice.nextcloud_folder_id, (folderId) => {
    if (folderId) {
        loadFiles();
    } else {
        files.value = null;
...
<template>
    <div>
        <!-- Loading -->
        <div v-if="state === 'loading'" class="text-sm text-gray-400 px-4 py-2">
        ...
        <div v-else-if="state === 'linked'">
            <div class="flex items-center justify-between px-4 py-2 border-b border-gray-100">
                <div class="flex items-center gap-2 text-sm text-gray-600">
                    ...
                    <span class="truncate max-w-64 text-xs">{{ advice.nextcloud_folder_path }}</span>
                </div>
                <Button variant="ghost" size="sm" :disabled="unlinking" @click="unlink">
                    <Link2Off class="h-4 w-4 mr-1 text-gray-400" />
        ...
        <!-- Broken link -->
        <div v-else-if="state === 'broken'" class="px-4 py-3 space-y-2">
        ...
        <!-- Dialogs -->
        <NextcloudCreateFolderDialog
            v-model:open="showCreateDialog"
            :advice-id="advice.id"
            :default-name="defaultFolderName"
        />
        <NextcloudLinkFolderDialog
            v-model:open="showLinkDialog"
            :advice-id="advice.id"
            :search-slug="defaultFolderName"
        />
    </div>
</template>

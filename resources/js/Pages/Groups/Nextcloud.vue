...
setLayoutProps({
    breadcrumbs: [
        { title: 'Initiativen' },
        { title: 'Initiativen-Verwaltung', href: route('groups.show', props.group.id) },
        { title: 'Nextcloud-Abgleich' },
    ],
});
...
<template>
    <div>
        <PageHeader
            :title="`Nextcloud-Abgleich — ${group.name}`"
            description="Berater*innen werden über die E-Mail Adresse und ihre Gruppen-Zuordnung abgeglichen"
        />

        <div class="bg-white shadow-sm sm:rounded-lg">
            <div class="space-y-4 p-6">
                <p v-if="group.nextcloud_group_name" class="text-sm text-gray-500">
                        ...
                        <TableBody>
                            <TableRow v-for="item in items" :key="item.nc_id ?? item.crm_user?.email">
                                ...
                                <TableCell class="text-right">
                                    ...
                                    <Button v-if="!item.crm_user && item.nc_id !== null" variant="outline" size="sm" @click="openImportDialog(item)">
